...
$seed_url = "https://api.craftypixels.com/seeds/11";
...
if ($random_category_name) {
    ...
    $search_url = "https://www.tripadvisor.com/Search?ssrc=A&q=top%20attraction%20in%20" . $slug . "%20" . $parent_slug . "&searchSessionId=6CFD68BA234093C1604C74B84D0587181640003067207ssid&sid=0E6A36ED2BEA4823BE3FBB06D786E1481640219902178&blockRedirect=true&queryParsed=true";
    echo $search_url . "\n";
    ...
    if ($ret == "success") {
        ...
        if ($html != null) {
            $NameOfResturant = "Top attractions in " . $random_category_name;
            foreach ($html->find('.location-meta-block') as $article) {
                ...
                if (
                    strpos($article->plaintext, $parent_name) !== false
                    && !(strpos($article->plaintext, 'Tours') !== false)
                    && !(strpos($article->plaintext, 'Taxis') !== false)
                    && !(strpos($article->plaintext, 'Bars') !== false)
                ) {
            ...
            if (!empty($content)) {
                $content = "Here are some top attractions in $random_category_name \n " . $content;
