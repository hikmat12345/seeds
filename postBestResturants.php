...
$seed_url = "https://api.craftypixels.com/seeds/7";
...
if ($random_category_name) {
    ...
    $search_url = "https://www.tripadvisor.com/Search?ssrc=e&q=best%20resturants%20in%20" . $slug . "%20" . $parent_name . "&searchSessionId=6CFD68BA234093C1604C74B84D0587181640003067207ssid&sid=0E6A36ED2BEA4823BE3FBB06D786E1481640219902178&blockRedirect=true&queryParsed=true";
    echo $search_url . "\n";
    ...
    if ($ret == "success") {
        ...
        if ($html != null) {
            $NameOfResturant = "Best restaurants in " . $random_category_name;
            foreach ($html->find('.location-meta-block') as $article) {
                $reviews = $article->find('.review_count', 0)->plaintext;

                // check if the article object has city or name in its content
            ...
            if (!empty($content)) {
                $content = "Here are some of the top rated restaurants in $random_category_name  \n" . $content;
